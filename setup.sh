...
OS_ID=${OS_ID//\"/}
echo "[INFO] Installing requirements"
sudo apt-get install ca-certificates software-properties-common certbot python3-certbot-apache "${APT_INSTALL_FLAGS[@]}"
case $OS_ID in
    ubuntu)
        echo "Ubuntu"
...
sudo apt update
sudo apt-get install apache2 curl php php-apcu php-common php-curl php-gd php-gmp php-imagick php-intl php-json php-mbstring php-memcache php-mysql php-zip mariadb-server mariadb-client "${APT_INSTALL_FLAGS[@]}"

php_versions=(7.4 8.0 8.1 8.2 8.3)
for version in "${php_versions[@]}"; do
    if apt-cache show "php$version" >/dev/null 2>&1; then
        sudo apt-get install "php$version" "php$version-fpm" "libapache2-mod-php$version" \
